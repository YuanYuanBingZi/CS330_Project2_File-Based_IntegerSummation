              ...
              unsigned long int* output, int num_ints)
{
  ...
  while(!feof(file1)){
  if( fgets (buffer1, MAX_NUM_LENGTH, file1)!=NULL ) {
      unsigned int x = atoi(buffer1);
      input_one[number1] = x;
      number1++;
   }
  }
  ...
  while(!feof(file2)){
  if( fgets (buffer2, MAX_NUM_LENGTH, file2)!=NULL ) {
      unsigned int x = atoi(buffer2);
      input_two[number2] = x;
      number2++;
   }
  }
                 ...
                 unsigned long int* output, int num_ints)
{
  ...
  char buffer3[MAX_NUM_LENGTH];

  for (int i = 0; i < num_ints; i++){
    sprintf(buffer3, "%lu", output[i]);
    fputs(buffer3, file3);
    fputs("\n", file3);
